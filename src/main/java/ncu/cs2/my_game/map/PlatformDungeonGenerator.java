...
public class PlatformDungeonGenerator {
    ...
    private static final int CLOSE_PARALLEL_GAP_TILES = 3;
    private static final double CLOSE_PARALLEL_OVERLAP_RATIO = 0.55;

    private record OptionalPlatform(Rectangle2D bounds, TileType type) { }
    private record RemovedPlatform(OptionalPlatform platform, List<Rectangle2D> rewards) { }
    ...
    private TileMap buildCandidate(int widthTiles, int levelIndex, PlatformGenerationConfig config,
        ...
        pruneBypassableRecoveryPlatforms(map, route, stats);
        pruneRedundantOptionalPlatforms(map, route, optionalPlatforms, stats);

        // Phase 4: hazards and content are allowed only once every terrain platform is final.
        PlatformValidationResult terrainValidation = validatePlatformMap(map, stats);
        if (!terrainValidation.spawnToExitReachable()) {
            return map;
        }
        map.clearEnemyZones();
        map.clearRewardZones();
        addTrapsAndDecorations(map, route, levelIndex);
        terrainValidation = validatePlatformMap(map, stats);
        if (!terrainValidation.spawnToExitReachable()) {
            return map;
        }
        addEnemyAndRewardZones(map, levelIndex, terrainValidation.unreachablePlatforms(), stats);
        return map;
    }
    ...
    private void addUpperRoute(TileMap map, List<Rectangle2D> route, int levelIndex,
        ...
        for (int attempt = 0; attempt < stoneCount * 6 && stoneCount > 0; attempt++) {
            ...
            if (!canPlaceOptionalConnector(map, startX, newY, pLength)) continue;
    ...
    private void addUpperFloatingWalls(TileMap map, List<Rectangle2D> route, int levelIndex) {
        PlacementValidator validator = new PlacementValidator(map);
        int target = 7 + Math.min(4, levelIndex / 2);
        int placed = 0;
        for (int attempt = 0; attempt < target * 22 && placed < target; attempt++) {
            int length = 5 + random.nextInt(6);
            int topY = 1 + random.nextInt(3);
            int capThickness = 1 + random.nextInt(2);
            int dropHeight = 3 + random.nextInt(5);
            int stemWidth = 2 + random.nextInt(2);
    ...
    private void pruneBypassableRecoveryPlatforms(TileMap map, List<Rectangle2D> route,
                                                  PlayerStats stats) {
        int floorEscapeY = map.getHeightTiles() - 3;
        boolean changed;
        do {
            changed = false;
            List<OptionalPlatform> candidates = collectRecoveryRelayCandidates(map);
            candidates.sort((a, b) -> Double.compare(b.bounds().getMinY(), a.bounds().getMinY()));
            for (OptionalPlatform candidate : candidates) {
                if (isRequiredPlatform(map, candidate.bounds())) continue;
                List<RemovedPlatform> removed = new ArrayList<>();
                ...
                if (stillPlayable) {
                    for (RemovedPlatform removedPlatform : removed) {
                        removePlatformReference(route, removedPlatform.platform().bounds());
                    }
                    changed = true;
                } else {
    ...
    private List<OptionalPlatform> addConnectorPlatforms(TileMap map, List<Rectangle2D> route,
        ...
        for (int i = 0; i < route.size() - 1 && extra > 0; i++) {
            ...
            if (canPlaceOptionalConnector(map, startX, midY, length)
    ...
    private boolean canPlaceOptionalConnector(TileMap map, int startX, int tileY, int length) {
        return canPlaceConnector(map, startX, tileY, length)
            && !wouldCreateCloseParallelPlatform(map, platformBounds(startX, tileY, length));
    }

    private boolean wouldCreateCloseParallelPlatform(TileMap map, Rectangle2D candidate) {
        for (Rectangle2D platform : map.getMainPlatforms()) {
            if (!isPhysicalPlatform(map, platform)) continue;
            double verticalTiles = Math.abs(candidate.getMinY() - platform.getMinY()) / TileMap.TILE_SIZE;
            if (verticalTiles < 0.5 || verticalTiles > CLOSE_PARALLEL_GAP_TILES) continue;
            double overlap = horizontalOverlap(candidate, platform);
            double smallerWidth = Math.min(candidate.getWidth(), platform.getWidth());
            if (smallerWidth > 0
                    && overlap >= Math.max(TileMap.TILE_SIZE * 2.0,
                        smallerWidth * CLOSE_PARALLEL_OVERLAP_RATIO)) {
                return true;
            }
        }
        return false;
    ...
    private void addBranches(TileMap map, List<Rectangle2D> route, int levelIndex,
        ...
        for (int attempt = 0; attempt < targetBranches * 5 && placed < targetBranches; attempt++) {
            ...
            if (startX < 2 || startX + length >= map.getWidthTiles() - 2) continue;
            if (!canPlaceOptionalConnector(map, startX, tileY, length)) continue;
    ...
    private void pruneRedundantOptionalPlatforms(TileMap map, List<Rectangle2D> route,
        ...
        do {
            ...
            addUniqueCandidates(candidates, collectCloseParallelPlatformCandidates(map));
            ...
            candidates.sort((a, b) -> {
                boolean aClose = hasCloseParallelPlatform(map, a.bounds());
                boolean bClose = hasCloseParallelPlatform(map, b.bounds());
                if (aClose != bClose) return aClose ? -1 : 1;
                return Double.compare(a.bounds().getMinY(), b.bounds().getMinY());
            });
            ...
            for (OptionalPlatform candidate : candidates) {
                ...
                if (!stillPlayable) {
                    for (Rectangle2D replacement : findWallTopReplacements(map, candidate.bounds())) {
                        wallTopReplacement = replacement;
                        map.addMainPlatform(replacement);
                        ...
                        map.removeMainPlatform(replacement);
                        wallTopReplacement = null;
                    }
                }
                if (stillPlayable) {
                    for (RemovedPlatform removedPlatform : removed) {
    ...
    private boolean removeOptionalPlatform(TileMap map, OptionalPlatform candidate,
                                           List<RemovedPlatform> removed) {
        if (!map.removePlatformRun(candidate.bounds())) return false;
        removed.add(new RemovedPlatform(candidate, map.removeRewardZonesOnPlatform(candidate.bounds())));
        return true;
    }
    ...
    private void cascadeRedundantOptionalPlatforms(TileMap map, PlayerStats stats,
        ...
        do {
            ...
            addUniqueCandidates(cascade, collectCloseParallelPlatformCandidates(map));
            addUniqueCandidates(cascade, collectRecoveryRelayCandidates(map));
    ...
    private List<OptionalPlatform> collectRecoveryRelayCandidates(TileMap map) {
        List<OptionalPlatform> candidates = new ArrayList<>();
        int minRecoveryY = map.getHeightTiles() - 8;
        for (Rectangle2D platform : new ArrayList<>(map.getMainPlatforms())) {
            int tileY = (int) Math.round(platform.getMinY() / TileMap.TILE_SIZE);
            if (tileY < minRecoveryY
                    || !isPhysicalPlatform(map, platform)
                    || isRequiredPlatform(map, platform)
                    || platformTileType(map, platform) != TileType.ONE_WAY_PLATFORM) {
                continue;
            }
            candidates.add(new OptionalPlatform(platform, TileType.ONE_WAY_PLATFORM));
        }
        return candidates;
    }

    private void addUniqueCandidates(List<OptionalPlatform> candidates,
                                     List<OptionalPlatform> additions) {
        for (OptionalPlatform addition : additions) {
            if (candidates.stream().noneMatch(candidate ->
                    samePlatform(candidate.bounds(), addition.bounds()))) {
                candidates.add(addition);
            }
        }
    }

    private List<OptionalPlatform> collectCloseParallelPlatformCandidates(TileMap map) {
        List<OptionalPlatform> candidates = new ArrayList<>();
        for (Rectangle2D platform : new ArrayList<>(map.getMainPlatforms())) {
            if (!isPhysicalPlatform(map, platform)
                    || isRequiredPlatform(map, platform)
                    || !hasCloseParallelPlatform(map, platform)) {
                continue;
            }
            candidates.add(new OptionalPlatform(platform, platformTileType(map, platform)));
        }
    ...
    private boolean hasCloseParallelPlatform(TileMap map, Rectangle2D platform) {
        for (Rectangle2D other : map.getMainPlatforms()) {
            if (samePlatform(platform, other) || !isPhysicalPlatform(map, other)) continue;
            double verticalTiles = Math.abs(platform.getMinY() - other.getMinY()) / TileMap.TILE_SIZE;
            if (verticalTiles < 0.5 || verticalTiles > CLOSE_PARALLEL_GAP_TILES) continue;
            double overlap = horizontalOverlap(platform, other);
            double smallerWidth = Math.min(platform.getWidth(), other.getWidth());
            if (smallerWidth > 0
                    && overlap >= Math.max(TileMap.TILE_SIZE * 2.0,
                        smallerWidth * CLOSE_PARALLEL_OVERLAP_RATIO)) {
                return true;
    ...
    private double horizontalOverlap(Rectangle2D a, Rectangle2D b) {
        return Math.max(0.0, Math.min(a.getMaxX(), b.getMaxX())
    ...
    private boolean isRequiredPlatform(TileMap map, Rectangle2D platform) {
        Rectangle2D exit = map.getExitBounds();
        double spawnFootY = map.getSpawnY() + Config.PLAYER_HEIGHT;
        if (platformContainsX(platform, map.getSpawnX())
                && Math.abs(spawnFootY - platform.getMinY()) <= TileMap.TILE_SIZE * 0.75) {
            return true;
        }
        if (exit != null) {
            double exitFootY = exit.getMaxY();
            double exitCenterX = exit.getMinX() + exit.getWidth() / 2.0;
            if (platformContainsX(platform, exitCenterX)
                    && Math.abs(exitFootY - platform.getMinY()) <= TileMap.TILE_SIZE * 0.75) {
                return true;
            }
        }
        return false;
    ...
    private List<Rectangle2D> findWallTopReplacements(TileMap map, Rectangle2D removedPlatform) {
        int platformY = (int) Math.round(removedPlatform.getMinY() / TileMap.TILE_SIZE);
        int left = Math.max(1, (int) Math.round(removedPlatform.getMinX() / TileMap.TILE_SIZE) - 3);
        int right = Math.min(map.getWidthTiles() - 2,
            (int) Math.round(removedPlatform.getMaxX() / TileMap.TILE_SIZE) + 2);
        int minY = Math.max(1, platformY);
        int maxY = Math.min(map.getHeightTiles() - 2, platformY + 6);

        List<Rectangle2D> replacements = new ArrayList<>();
        for (int y = minY; y <= maxY; y++) {
            ...
            while (x <= right) {
                ...
                int length = x - start;
                if (length >= 2) {
                    replacements.add(platformBounds(start, y, length));
                }
            }
        }
        double platformCenter = removedPlatform.getMinX() + removedPlatform.getWidth() / 2.0;
        replacements.sort((a, b) -> {
            int yCompare = Double.compare(a.getMinY(), b.getMinY());
            if (yCompare != 0) return yCompare;
            double aCenter = a.getMinX() + a.getWidth() / 2.0;
            double bCenter = b.getMinX() + b.getWidth() / 2.0;
            return Double.compare(Math.abs(aCenter - platformCenter), Math.abs(bCenter - platformCenter));
        });
        return replacements;
    }
    ...
    private void restoreOptionalPlatforms(TileMap map, List<RemovedPlatform> removed) {
        for (int i = removed.size() - 1; i >= 0; i--) {
            RemovedPlatform removedPlatform = removed.get(i);
            restoreOptionalPlatform(map, removedPlatform.platform());
            for (Rectangle2D reward : removedPlatform.rewards()) {
                map.addRewardZone(reward);
            }
        }
    }
    ...
    private void addEnemyAndRewardZones(TileMap map, int levelIndex,
                                        List<Rectangle2D> unreachablePlatforms,
                                        PlayerStats stats) {
        int enemies = levelIndex == 1 ? 2 : 5;
        PlacementValidator validator = new PlacementValidator(map);
        List<Rectangle2D> surfaces = collectContentSpawnSurfaces(map, stats);
        int step = Math.max(2, surfaces.size() / (levelIndex == 1 ? 3 : 6));
        for (int i = 1; i < surfaces.size() - 1 && enemies > 0; i += step) {
            Rectangle2D platform = surfaces.get(i);
        ...
        for (int i = 1; i < surfaces.size() - 1; i += 4) {
            Rectangle2D platform = surfaces.get(i);
            if (unreachablePlatforms.contains(platform)) continue;
    ...
    private List<Rectangle2D> collectContentSpawnSurfaces(TileMap map, PlayerStats stats) {
        List<Rectangle2D> surfaces = new ArrayList<>(map.getMainPlatforms());
        for (Rectangle2D wallTop : collectClearWallTopSurfaces(map, 3)) {
            if (!containsPlatform(surfaces, wallTop)) {
                surfaces.add(wallTop);
            }
        }
        List<Rectangle2D> useful = collectPlatformsLeadingToGoal(map, surfaces, stats);
        List<Rectangle2D> result = new ArrayList<>();
        for (Rectangle2D surface : useful) {
            if (surface.getMinX() < TileMap.TILE_SIZE * 4) continue;
            Rectangle2D exit = map.getExitBounds();
            if (exit != null && Math.abs((surface.getMinX() + surface.getMaxX()) / 2.0
                    - (exit.getMinX() + exit.getMaxX()) / 2.0) < TileMap.TILE_SIZE * 4) {
                continue;
            }
            result.add(surface);
        }
        result.sort((a, b) -> {
            int xCompare = Double.compare(a.getMinX(), b.getMinX());
    ...
    private List<Rectangle2D> collectClearWallTopSurfaces(TileMap map, int minimumLength) {
        List<Rectangle2D> wallTops = new ArrayList<>();
        for (int y = 2; y < map.getHeightTiles() - 2; y++) {
            int x = 1;
            while (x < map.getWidthTiles() - 1) {
                ...
                if (length >= minimumLength) {
                    wallTops.add(platformBounds(start, y, length));
                }
            }
        }
        return wallTops;
